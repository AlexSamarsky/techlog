...
class LogEventAnalyze(LogBase):
    ...
    def main_process(self, process_path: str, log_event: TechLogEvent) -> None:
        # if log_event.file.full_path == 'logs_test/bill/events/t_connectID_35581.log':
        #     c = 1
        ...
        if log_event.name == self._begin_event_name:
            ...
            if event_process_analyze.start_time:
                ...
                start_time = datetime.strftime(event_process_analyze.start_time, TimePatterns.format_time_full)
                end_time = datetime.strftime(event_process_analyze.end_time, TimePatterns.format_time_full)
                if end_time < start_time:
                    raise ValueError ('Ошибка анализа')
                event_process_analyze.tech_log_event.text = event_process_analyze.tech_log_event.text.rstrip('\n') \
                                                            + f',tla:start_time={start_time}' \
                                                            + f',tla:end_time={end_time}' \
                                                            + f',tla:duration={event_process_analyze.tech_log_event.duration}' \
                                                            + f',tla:path={self._path}' \
                                                            + f',tla:rel_path={log_event.file.stem}' \
                                                            + f',tla:uuid={uuid.uuid4()}' \
                                                            + '\n'
    ...
    #     super().execute_end_process_file(file_object)
    #     self
